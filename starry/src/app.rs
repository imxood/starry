...
pub fn start() {
    ...
    #[cfg(target_arch = "wasm32")]
    app.add_plugin(crate::plugins::web::WebPlugin);
...
fn resize_notificator(resize_event: Res<Events<WindowResized>>, mut windows: ResMut<Windows>) {
    ...
    for e in reader.iter(&resize_event) {
        let window = windows.get_mut(e.id).unwrap();
        if window.width() != e.width || window.height() != e.height {
            window.set_resolution(e.width, e.height);
            log::info!("width = {} height = {}", e.width, e.height);
        }
    }
}
